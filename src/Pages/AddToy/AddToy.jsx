...
const AddToy = () => {
    ...
    useEffect(() => {
        fetch('https://the-toy-shop-server.vercel.app/categories')
            .then(res => res.json())
    ...
    const handleAddToy = (event) => {
        ...
        fetch('https://the-toy-shop-server.vercel.app/myToys', {
            method: 'POST',
            headers: {
        ...
        <div className='card-body bg-gray-300 mb-8 rounded-xl'>
            ...
            <form onSubmit={handleAddToy} >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <select className='input input-bordered' name="categoryName" id="">{categories.map(cat => <option key={cat._id} value={cat.name}>{cat.name}</option>)}</select>
